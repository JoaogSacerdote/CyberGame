...
#define PFRAME_W   32
#define PFRAME_H   48
#define PSTEP      2                /* pixels por tick (UI_TICK_MS=100 -> 20px/s) */
...
static int8_t  s_dir = 0;           /* 0=DOWN, 1=RIGHT, 2=LEFT, 3=UP */
...
static void recepcao_tick(lv_timer_t *t)
{
    ...
    /* joystick: y positivo = pra CIMA (logica do hal). Tela: y cresce pra baixo. */
    if (j.y >  J_DEADZONE) dy = -1;
    else if (j.y < -J_DEADZONE) dy = +1;

    /* Atualiza direcao do sprite — eixo dominante */
    if (abs(dx) > abs(dy)) {
        s_dir = (dx > 0) ? 1 /*RIGHT*/ : 2 /*LEFT*/;
    } else if (dy != 0) {
    ...
    if (near_npc != s_npc_facing) {
        ...
        if (near_npc) {
            ...
            ESP_LOGI(TAG, "near NPC -> dialog pose");
        } else {
            lv_image_set_src(s_npc, &img_rec_recep_idle);
            lv_obj_set_pos(s_npc, IMG_REC_RECEP_IDLE_META.off_x,
                                  IMG_REC_RECEP_IDLE_META.off_y);
            ESP_LOGI(TAG, "far from NPC -> idle pose");
        }
    }

    /* Icone notif: pisca 500ms on/off enquanto visivel; some + inicia
     * dialogo quando jogador chega no NPC pela primeira vez. */
    if (s_icon_visible) {
        if (near_npc) {
    ...
    /* Gatilho de porta -> muda pra Empresa */
    if (g && g->kind == AREA_PORTA_EMPRESA) {
        ESP_LOGI(TAG, "porta empresa -> trocando sala");
        fsm_set_gameplay_sala(GAMEPLAY_SALA_EMPRESA);
        /* engine_task vai detectar a mudanca e chamar ui_show_empresa() */
    }
}
...
void screen_recepcao_build(void)
{
    ...
    s_dir = 0; s_walk_idx = 1; s_walk_ms = 0;
    lv_obj_set_pos(s_player, s_px, s_py);
    apply_player_frame();
